...
<body>
    <form method="post" action="<?php echo base_url('welcome/actualizar_programacion');?>">
        <div class="mb-3">
        <label for="floatingInput1" class="form-label">Id</label>
        <input type="text" name="id" id="floatingInput1" readonly class="form-control" placeholder="Id..." value="<?php echo $programacion->id_programacion; ?>">
        </div>
        <div class="mb-3">
        <label for="floatingInput2"  class="form-label">Fecha</label>
        <input type="date" name="fecha" id="floatingInput2" class="form-control" placeholder="Fecha..." required value="<?php echo $programacion->fecha_curso; ?>">
        </div >
        <div class="mb-3">
        ...
        <select class="form-select" aria-label="Default select example" name="curso" id="floatingInput3"> 
            <?php foreach ($cursos as $key => $value):?>
                <?php if( $programacion->curso == $value->id_curso):?>
                    <option selected hidden value="<?php echo $value->id_curso ?>"><?php echo $value->descripcion_curso ?></option> 
                <?php endif;?>
        ...
        <?php foreach ($aulas as $key => $value):?>
                <?php if( $programacion->aula == $value->id_aula):?>
                    <option selected hidden value="<?php echo $value->id_aula ?>"><?php echo $value->descripcion_aula ?></option> 
                <?php endif;?>
        ...
        <?php foreach ($profesores as $key => $value):?>
                <?php if($programacion->profesor == $value->id_profesor):?>
                    <option selected hidden value="<?php echo $value->id_profesor ?>"><?php echo $value->nombre_profesor ?></option> 
                <?php endif;?>
        ...
        <?php foreach ($estudiantes as $key => $value):?>
                <?php if( $programacion->estudiante == $value->id_estudiante):?>
                    <option selected hidden value="<?php echo $value->id_estudiante ?>"><?php echo $value->nombre_estudiante ?></option> 
                <?php endif;?>
